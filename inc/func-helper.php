 ...
 * @return mixed
 */
function ifield( $key, $post_id = false ) {
...
function is_reg_open() {
	...
	if ( $qAllAngakatan->have_posts() ) {
		...
		while ( $qAllAngakatan->have_posts() ):
			...
			$pembukaan_timestamp   = ifield( 'pembukaan' );
			...
			$ikhwan_kuota = ifield( 'kuota_ikhwan' );
			$ikhwan_isi   = ifield( 'ikhwan_isi' ) ? ifield( 'ikhwan_isi' ) : 0;
			$ikhwan_sisa  = $ikhwan_kuota - $ikhwan_isi;
			$akhwat_kuota = ifield( 'kuota_akhwat' );
			$akhwat_isi   = ifield( 'akhwat_isi' ) ? ifield( 'akhwat_isi' ) : 0;
 ...
 * @return string
 */
function generate_random_string( $length = 7, $numbers_only = false ) {
 ...
 * @param $ujk
 */
function update_isi_angkatan( $angkatan_id, $ujk ) {
	$status = is_reg_open();
	switch ( $ujk ) {
		case '1':
 ...
 */
function update_fields( $post_id, $args, $update_user = false ) {
	if ( $update_user ) {
 ...
 * @return bool|mixed
 */
function generate_kelas( $users, $angkatan, $jenis_kelamin = 1 ) {
	$result     = false;
	$prefix_jk  = $jenis_kelamin == 1 ? "I" : "A";
	$nama_kelas = PREFIX . $angkatan . "-" . $prefix_jk . generate_random_string( 5, true );
